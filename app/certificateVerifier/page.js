...
// components/CertificateVerifier.js
"use client";
...
export default function CertificateVerifier() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-6">
      <div className="bg-white shadow-lg rounded-2xl p-8 w-full max-w-lg text-center text-black">
        <h1 className="text-2xl font-bold mb-6">🔎 Unified Certificate Verification</h1>
        ...
        <button
          onClick={handleVerify}
          disabled={isLoading || !file}
          className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg disabled:bg-gray-400 w-full"
        >
          {isLoading ? 'Verifying...' : 'Verify Certificate'}
        </button>
          ...
          <div className="mt-4 text-left text-sm bg-gray-50 p-4 rounded">
            {result.status === 'VALID' && (
              <div className="text-green-600">
                <h3 className="font-bold text-lg">✅ Valid Certificate</h3>
                <p className="text-xs text-gray-500 font-mono">Verified via: {result.method}</p>
                <p><strong>Name:</strong> {result.certificate.name}</p>
                <p><strong>Course:</strong> {result.certificate.course}</p>
                <p><strong>SGPA:</strong> {result.certificate.sgpa}</p>
                <p><strong>CGPA:</strong> {result.certificate.cgpa}</p>
              </div>
            )}
            {result.status === 'INVALID' && (
              <div className="text-red-600">
                <h3 className="font-bold text-lg">❌ Verification Failed</h3>
                <p className="text-xs text-gray-500 font-mono">Method: {result.method}</p>
                <p>{result.details}</p>
              </div>
            )}
            {result.status === 'ERROR' && (
            ...
            {result.ocrText && (
                <details className="mt-4 cursor-pointer">
                    <summary className="font-semibold text-gray-700">Show Raw OCR Text</summary>
                    <pre className="whitespace-pre-wrap bg-gray-200 p-2 rounded mt-2 text-xs text-gray-800">{result.ocrText}</pre>
                </details>
            )}
          </div>
        )}
